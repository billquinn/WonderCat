...
write.csv(dataframe, paste0(getwd(), "/main/wonderCat.csv"), row.names = FALSE)

dataframe <- read_csv(paste0(getwd(), "/main/wonderCat.csv"), col_names = TRUE)
colnames(data) <- c(
      'id', 'author', 'date', 'benefit', 'experience', 'technology', 'text', 'QID'
    )

# wikiResp <- get_wikidata(dataframe)
wikiResp <- read_csv(paste0(getwd(), "/main/wikiData.csv"), col_names = TRUE)
colnames(wikiResp) <- c(
  'QID', 'title', 'pubDate', 'genreLabel', 'countryOriginLabel', 'coordinates', 'lon', 'lat'
)
wikiResp$coordinates <- gsub('(.*),.*', '\\1', wikiResp$coordinates)
wikiResp$lon <- gsub('(.*),.*', '\\1', wikiResp$lon)
wikiResp$lat <- gsub('(.*),.*', '\\1', wikiResp$lat)
...
net <- create_full_network_data(dataframe)
```
